...
public class RegisterController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (path.contains("register")) {
			resp.sendRedirect("/newsWeb/views/user/registerpage.jsp");
		}
	}
...
//			String email = req.getParameter("email");
			...
			if (service.checkPasswordAndConfirmPassword(password, confirmPassword) == false) {
				resp.sendRedirect("/newsWeb/user/register?error=password_mismatch");
				return;
			} else {
				if (service.checkPasswordFormat(confirmPassword) == false) {
					resp.sendRedirect("/newsWeb/user/register?error=invalid_password_format");
					return;
				}
			}

			if (service.checkFormatMail(email) == false) {
				resp.sendRedirect("/newsWeb/user/register?error=invalid_email_format");
				return;
			}
			if (!service.checkFormatPhone(phone) == false) {
				resp.sendRedirect("/newsWeb/user/register?error=invalid_phone_format");
				return;
			}

			if (service.checkExistEmail(email)) {
				resp.sendRedirect("/newsWeb/user/register?error=email_exist");
				return;
			}

			if (service.checkExistPhoneNumber(phone)) {
				resp.sendRedirect("/newsWeb/user/register?error=phone_exist");
				return;
			}
